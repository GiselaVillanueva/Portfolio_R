...
# Por favor, escrib? tu nombre y apellido a continuaci?n:
# Gisela Villanueva

# Por favor, escrib? la fecha de entrega de este challenge
# (en formato A?O.MES.D?A):
# 2024.02.06


# Introducci?n ------------------------------------------------------------

# ?Hola! ?Gracias por tomarte el tiempo para participar!
#   
# Este peque?o "desaf?o" est? dise?ado para que puedas mostrar
# brevemente como pens?s y encar?s ciertos problemas.
# 
# A continuaci?n se presentar?n una serie de consignas b?sicas
# a resolver.
# Le? cuidadosamente cada punto y tomate el tiempo que necesites
# para resolverlo. No hay "preguntas trampa", pero s? es importante
# que sigas las consignas. Existen varias maneras de resolverlos, y todas
# son aceptadas mientras se concrete el objetivo.
# 
# Si cre?s que es necesario alguna aclaraci?n en tus respuestas,
# por favor dej? un comentario dentro del c?digo, respetando las
# separaciones de los comentarios preestablecidas.


# Conexi?n ----------------------------------------------------------------

# Este punto es para que muestres tu habilidad usando SQL,
# por lo que se pide su resoluci?n mediante la utilizaci?n de
# "queries" ?nicamente (es decir, sin la ayuda de herramientas
# externas de an?lisis, tales como librer?as o comandos de R).
# 
# Para comenzar, es necesario conectarse a la base de datos
# sqlite "clarinds.db".


#### Tutorial conexi?n a la base de datos:
#   
#   A continuaci?n explicaremos brevemente como conectarse a esta
#   base de datos:
#   
#   En primer lugar verific? que el archivo "clarinds.db"
#   se encuentre en el mismo directorio que este script:
#   
#   Luego comprobemos que la conexi?n funciona correctamente
#   ejecutando las siguientes l?neas a continuaci?n:
...
# Cambio la ruta a donde est? este script

setwd("C:/Users/gisel/Downloads/2024_Clarin_Challenge/")

# Para conectarme a la base
con <- DBI::dbConnect(dbDriver("SQLite"),    
                      dbname = "clarinds.db")
...
# El resultado de la query ejecutada deber?a retornar un dataframe con
# los nombres de las tablas que la base de datos contiene. Las tablas son:
#   
# summary
# notas
# navegacion
#
# Para poder hacer los ejercicios solicitados se puede utilizar
# la siguiente estructura:

query_de_ejemplo <- ("
  SELECT ... <tu c?digo ac?>
             ")
respuesta_pregunta <- dbGetQuery(con, query_de_ejemplo)
...
# Si quer?s dejarnos alg?n comentario o sugerencia, fuera de las consignas
# y sus resoluciones, te invitamos a que lo hagas a continuaci?n.
